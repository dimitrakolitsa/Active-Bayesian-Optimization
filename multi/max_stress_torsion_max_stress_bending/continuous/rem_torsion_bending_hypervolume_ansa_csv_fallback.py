# FILE: live_moo_bo.py
# PURPOSE: Live, Multi-Objective BO using the ANSA IAP approach.
...
RESULTS_CSV_PATH = "moo_bo_results_with_fallback.csv"
...
# --- Simple Pairwise Non-Dominated Check (Minimization) ---
...
# --- MODIFIED: Prepare and Save Initial Data to CSV ---
...
# NEW: Add acquisition function column for initial data
df_for_csv['acquisition_function'] = 'N/A'

# MODIFIED: Define the full header order with the new column
...
hypervolume_history = []
hv_ref_point_raw = torch.tensor([1500.0, 1500.0], dtype=torch.float64)
...
print(f"\nStarting Live Multi-Objective Bayesian Optimization...")
...
try:
    ...
    for i in range(MAX_TOTAL_ITERS):
        ...
        if exploration_iters_left > 0:
            ...
            # Define the scalarized LCB acquisition function
            def scalarized_lcb(X):
                posterior = model.posterior(X)
                means, stds = posterior.mean, posterior.variance.clamp_min(1e-9).sqrt()
                # LCB for each objective (BoTorch maximizes, so this is correct)
                ...
                return (0.5 * lcb_obj1 + 0.5 * lcb_obj2).squeeze(-1)
        ...
        try:
            next_x_scaled, _ = optimize_acqf(acq_function_to_use, bounds=scaled_bounds, q=1, num_restarts=15, raw_samples=2048)
        except Exception as e:
            print(f"  FATAL: Acquisition function optimization failed: {e}. Stopping."); break
        ...
        print(f"  Received result: {objective_target_name_1}={new_obj1_raw:.4f}, {objective_target_name_2}={new_obj2_raw:.4f}")
        ...
        print(f"   Current Hypervolume: {current_hv:.4f}, Pareto Front Size: {pareto_front_raw.shape[0]}")

        # --- NEW: Stagnation Check Logic ---
        if len(hypervolume_history) > STAGNATION_WINDOW:
            past_hv = hypervolume_history[-(STAGNATION_WINDOW + 1)]
            
            # Check if hypervolume has improved beyond a small tolerance
            ...
            # If we are stalled and not already exploring, trigger exploration
...
# Final Pareto Front from BO Results
...
if final_pareto_points_raw.shape[0] > 0:
    print("Objectives (Raw):"); print(pd.DataFrame(final_pareto_points_raw.numpy(), columns=[objective_target_name_1, objective_target_name_2]))
...
legend_elements = [
    Line2D([0], [0], color='w', marker='', linestyle='',
           label=f'Max Hypervolume Achieved: {max_hv_achieved:.4f}')
]
...
ax1.set_title("Hypervolume Convergence Plot")
ax1.grid(True, which='both', linestyle='--')
ax1.set_xlim(left=0)
ax1.set_ylim(bottom=0)
plt.tight_layout()
plt.savefig('live_moo_hypervolume_convergence_with_fallback.png')
print("\nSaved plot to 'live_moo_hypervolume_convergence_with_fallback.png'")
plt.show()

# 2. Final Objective Space
plt.figure(figsize=(10, 8))
plt.scatter(final_objectives_raw[:, 0].numpy(), final_objectives_raw[:, 1].numpy(), c='blue', alpha=0.6, s=25, label='All Evaluated Points')
if final_pareto_points_raw.shape[0] > 0:
    plt.scatter(final_pareto_points_raw[:, 0].numpy(), final_pareto_points_raw[:, 1].numpy(), c='lime', s=100, edgecolor='black', marker='*', label='Final Pareto Front', zorder=3)
plt.xlabel(f"{objective_target_name_1} (Minimize)")
plt.ylabel(f"{objective_target_name_2} (Minimize)")
plt.title("Objective Space - Live Optimization Results")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig('live_moo_objective_space_with_fallback.png')
print("Saved plot to 'live_moo_objective_space_with_fallback.png'")
plt.show()