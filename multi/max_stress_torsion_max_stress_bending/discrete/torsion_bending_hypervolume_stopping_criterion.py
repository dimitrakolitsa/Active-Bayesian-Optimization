...
# --- Simple Pairwise Non-Dominated Check (Minimization) for pareto front finding (O(N^2)) ---
...
obj_indices = [0, 1]
print(f"\nOptimizing objectives: Minimize {objective_target_name_1} and {objective_target_name_2} (Unconstrained)")
    ...
    for i in range(num_outputs):
        ...
        try:
            with gpytorch.settings.cholesky_jitter(1e-4):
                fit_gpytorch_mll(mll_i, max_retries=5, options={'maxiter': 150})
...
def find_closest_candidate(x_proposed_unscaled, X_cands, Y_cands, obj_idx_1, obj_idx_2):
    distances = torch.norm(X_cands - x_proposed_unscaled.squeeze(0), dim=1) #euclidean
    ...
    return closest_index.item(), closest_X, torch.tensor([[ -raw_obj1]]), torch.tensor([[ -raw_obj2]])
...
selection_methods = [] # list to track the point selection method in each iteration

# --- Reference Point ---
hv_ref_point_raw = torch.tensor([1500.0, 1500.0], dtype=torch.float64) # fixed reference point => worse/greater than all points
...
# --- Calculate Initial Hypervolume with simple_is_non_dominated ---
initial_objectives_raw = Y_init[:, [objective_index_1, objective_index_2]]
initial_hv = 0.0; initial_pareto_raw = torch.empty((0, 2), dtype=initial_objectives_raw.dtype)
try:
...
for i in range(MAX_TOTAL_ITERS):
    ...
    fallback_used_this_iter = False # reset lcb flag
    ...
    try:
        ...
        selection_methods.append('Error') # store error
        continue

    print("  Setting up EHVI acquisition function...")
    acqf = None
    try:
        if train_y_standardized.shape[0] > 0:
             partitioning = FastNondominatedPartitioning(ref_point=hv_ref_point_std_neg, Y=train_y_standardized)
        else:
             print("Warning: No training data for EHVI partitioning. Skipping acquisition.")
             hypervolume_history.append(hypervolume_history[-1] if hypervolume_history else 0.0)
             selection_methods.append('Error') # store error
             continue
        acqf = ExpectedHypervolumeImprovement(
            model=model, ref_point=hv_ref_point_std_neg.tolist(), partitioning=partitioning
        )
    except Exception as e:
    ...
    if acqf is not None:
        ...
        try:
            next_x_scaled, acq_value = optimize_acqf(
                ...
                num_restarts=15, raw_samples=2048,
    ...
    if next_x_scaled is not None: # EHVI potentially successful
        closest_idx, _, next_y_obj1_raw, next_y_obj2_raw = find_closest_candidate(
            next_x_unscaled, X_candidates, Y_candidates, objective_index_1, objective_index_2
        )
        if closest_idx in original_indices.tolist():
            ...
            # fallback_used_this_iter remains False -> EHVI selection
    else:
        # use Fallback because EHVI setup/opt failed OR next_x_scaled started as None
        fallback_used_this_iter = True

    # Fallback LCB
    if next_x_scaled is None:
        fallback_used_this_iter = True # lcb fallback used
        if acqf is None: print("   Executing fallback because EHVI creation failed.")
        elif not candidate_already_evaluated: print("   Executing fallback (optimize_acqf failed): Scalarized LCB.")

        unevaluated_indices_all = [idx for idx in range(len(X_candidates)) if idx not in original_indices.tolist()]
        if not unevaluated_indices_all: print("   No unevaluated candidates left. Terminating early."); break
        unevaluated_indices_tensor = torch.tensor(unevaluated_indices_all, dtype=torch.long)
        print(f"   Number of candidates for scalarized LCB fallback: {len(unevaluated_indices_tensor)}")

        with torch.no_grad():
            max_fallback_cands = 5000 #sample randomly 5000 not evaluated candidates and do lcb on them to speed up posterior mean calculation from gp
            if len(unevaluated_indices_tensor) > max_fallback_cands:
                print(f"   Limiting fallback candidates from {len(unevaluated_indices_tensor)} to {max_fallback_cands} (random subset)")
                perm = torch.randperm(len(unevaluated_indices_tensor))
                sample_indices = perm[:max_fallback_cands]
                unevaluated_indices_tensor_subset = unevaluated_indices_tensor[sample_indices]
                unevaluated_X_scaled_fallback = X_candidates_scaled[unevaluated_indices_tensor_subset]
            else:
                 unevaluated_indices_tensor_subset = unevaluated_indices_tensor
                 unevaluated_X_scaled_fallback = X_candidates_scaled[unevaluated_indices_tensor_subset]

            print(f"   Calculating posterior for {len(unevaluated_X_scaled_fallback)} fallback candidates...")
            posterior = model.posterior(unevaluated_X_scaled_fallback)
            means_standardized = posterior.mean
            variances_standardized = posterior.variance
            stds_standardized = variances_standardized.sqrt()

        mean_obj1_std = means_standardized[:, 0]
        mean_obj2_std = means_standardized[:, 1]
        std_obj1_std = stds_standardized[:, 0]
        std_obj2_std = stds_standardized[:, 1]
        beta_lcb = 2.0
        w1, w2 = 0.5, 0.5
        scalarized_lcb_std = w1 * (mean_obj1_std - beta_lcb * std_obj1_std) + w2 * (mean_obj2_std - beta_lcb * std_obj2_std)

        best_lcb_idx_in_subset = torch.argmax(scalarized_lcb_std)
        closest_idx = unevaluated_indices_tensor_subset[best_lcb_idx_in_subset].item()

        closest_cand_X = X_candidates[closest_idx].unsqueeze(0)
        closest_cand_X_scaled = torch.tensor(x_scaler.transform(closest_cand_X.numpy()), dtype=torch.float64)
        raw_obj1 = Y_candidates[closest_idx, objective_index_1].item(); raw_obj2 = Y_candidates[closest_idx, objective_index_2].item()
        next_y_obj1_raw = torch.tensor([[ -raw_obj1]]); next_y_obj2_raw = torch.tensor([[ -raw_obj2]])
    ...
    next_y_combined_raw = torch.cat([next_y_obj1_raw, next_y_obj2_raw], dim=1)
    ...
    # --- Update and Record Hypervolume with simple_is_non_dominated ---
    ...
    current_hv = 0.0
    pareto_front_raw = torch.empty((0, 2), dtype=all_objectives_raw.dtype)
    try:
    ...
    print(f"   Current Hypervolume: {current_hv:.4f}")
    ...
    if i >= BASE_ITERS - 1:
        # Check if we have enough data points in the history to look back
        if len(hypervolume_history) > PATIENCE_WINDOW:
            # Value from PATIENCE_WINDOW iterations ago
            past_hv = hypervolume_history[-(PATIENCE_WINDOW + 1)]
            current_hv_check = hypervolume_history[-1]
            
            # Avoid division by zero if the past hypervolume was zero
            if abs(past_hv) > 1e-9:
                ...
                if relative_improvement < REL_IMPROVEMENT_TOL:
                    ...
                    break
            # Fallback for near-zero past HV: check absolute improvement
            elif (current_hv_check - past_hv) < 1e-7: # A small absolute tolerance
...
final_pareto_points_raw = None
final_pareto_features_unscaled = None
print("\n--- Calculating Final BO Pareto Front ---")
try:
    if final_objectives_raw.shape[0] > 0:
        print(f"DEBUG: Calculating non-dominated on final RAW BO objectives using simple check (shape: {final_objectives_raw.shape})")
        ...
        if final_pareto_points_raw.shape[0] > 0:
            final_pareto_features_scaled = train_x_scaled[non_dominated_mask_final]
            ...
            print("Objectives (Raw):"); df_bo_pareto = pd.DataFrame(final_pareto_points_raw.numpy(), columns=[objective_target_name_1, objective_target_name_2]); print(df_bo_pareto)
...
# --- Determine True Pareto Front ---
print("\n--- Determining True Pareto Front from Initial + Candidate Data (using Simple Check) ---")
Y_init_objectives_raw = Y_init[:, [objective_index_1, objective_index_2]]
Y_candidates_objectives_raw = Y_candidates[:, [objective_index_1, objective_index_2]]
...
try:
    precision_dup = 1e5; df_true_space = pd.DataFrame(true_space_objectives.numpy(), columns=['o1', 'o2'])
    ...
    unique_true_space_df = df_true_space.drop_duplicates(subset=['r1', 'r2'])
    unique_true_space_objectives = torch.tensor(unique_true_space_df[['o1', 'o2']].values, dtype=torch.float64)
...
try:
    if unique_true_space_objectives.shape[0] > 0:
        ...
        true_pareto_front_raw = unique_true_space_objectives[non_dominated_mask_true]
        end_time = time.time()
        ...
        if true_pareto_front_raw.shape[0] > 0:
            df_true_pareto = pd.DataFrame(true_pareto_front_raw.numpy(), columns=[objective_target_name_1, objective_target_name_2])
            print("True Pareto Front (Raw - Simple Check):")
            print(df_true_pareto)
            negated_true_pareto = -true_pareto_front_raw
            negated_ref_point = -hv_ref_point_raw
            hv_calculator_true = Hypervolume(ref_point=negated_ref_point)
            true_hv = hv_calculator_true.compute(negated_true_pareto)
            print(f"True Maximum Hypervolume (Simple Check): {true_hv:.4f}")
            final_bo_hv = hypervolume_history[-1] if hypervolume_history else 0.0
            print(f"Final BO Hypervolume (from history): {final_bo_hv:.4f}")
            if true_hv > 1e-9:
            ...
            print("\nTrue Pareto front determined to be empty (Simple Check).")
            true_hv = 0.0
    else: 
        print("\nNo unique combined points (Init+Candidates) found.")
        true_hv = 0.0
...
plt.plot(range(len(hypervolume_history)), hypervolume_history, marker='o', linestyle='-', label='BO Hypervolume')
...
plt.ylim(bottom=0)
plt.tight_layout()
plt.savefig('hypervolume_convergence.png')
plt.show()

# 2. Plot Final Objective Space 
plt.figure(figsize=(10, 8))
if train_y_standardized.shape[0] > 0:
    all_objectives_raw_plot = final_objectives_raw # Use final objectives from BO results
    # Plot all evaluated points in blue
    plt.scatter(all_objectives_raw_plot[:, 0].numpy(), all_objectives_raw_plot[:, 1].numpy(),
                c='blue', alpha=0.6, s=20, label='All Evaluated Points')
    
else: print("\nNo points evaluated to plot in objective space.")

# Plot BO Pareto front points as green stars
if final_pareto_points_raw is not None and final_pareto_points_raw.shape[0] > 0:
    plt.scatter(final_pareto_points_raw[:, 0].numpy(), final_pareto_points_raw[:, 1].numpy(),
                c='lime', s=100, edgecolor='black', marker='*', label='BO Pareto Front', zorder=3)
    
# Plot True Pareto front points as red circles
if true_pareto_front_raw is not None and true_pareto_front_raw.shape[0] > 0:
    plt.scatter(true_pareto_front_raw[:, 0].numpy(), true_pareto_front_raw[:, 1].numpy(),
                facecolors='none', edgecolors='red', s=60, linewidth=1.5, label='True Pareto Front (Simple Check)', zorder=2)
ax2 = plt.gca()
handles2, labels2 = ax2.get_legend_handles_labels()
if handles2:
    ax2.legend(loc='best') #legend place
plt.xlabel(f"{objective_target_name_1} (Minimize)")
plt.ylabel(f"{objective_target_name_2} (Minimize)")
plt.title("Objective Space - Pareto Front Comparison")
plt.grid(True)
plt.tight_layout()
...
# 3. Plot Selection Method over iterations
plt.figure(figsize=(12, 6)) 
if len(selection_methods) > 0: 
    bo_iterations = list(range(1, len(selection_methods) + 1))
    # make sure hypervolume history aligns (it has n+1 points, selection methods has n)
    if len(hypervolume_history) == len(selection_methods) + 1:
        bo_hypervolumes = hypervolume_history[1:] # exclude initial HV
    else:
        print("Warning: Mismatch between hypervolume history and selection methods. Cannot plot breakdown.")
        bo_hypervolumes = [] 

    if len(bo_iterations) == len(bo_hypervolumes): # make sure lengths match
        plt.plot(bo_iterations, bo_hypervolumes, color='darkgrey', linestyle='-', zorder=1, alpha=0.7, label='_nolegend_')
        color_map = {'EHVI': '#E41A1C', 'LCB': '#377EB8', 'Error': 'grey'} # Red for EHVI, Blue for LCB
        ...
        print("Warning: Mismatch between BO iterations and hypervolumes for plotting selection breakdown.")
...
#custom Legend
legend_elements = [
    Line2D([0], [0], marker='o', color='w', label='Selected via EHVI', markerfacecolor='#E41A1C', markeredgecolor='grey', markersize=8),
    Line2D([0], [0], marker='o', color='w', label='Selected via LCB Fallback', markerfacecolor='#377EB8', markeredgecolor='grey', markersize=8)
]
if true_pareto_front_raw is not None and true_pareto_front_raw.shape[0] > 0 and true_hv > 1e-9:
...
plt.title("BO Hypervolume Progression by Selection Method", fontsize=14)
plt.grid(True, which='both', linestyle='--', linewidth=0.5, alpha=0.6)
plt.xticks(fontsize=10); plt.yticks(fontsize=10)
plt.xlim(left=0) # Start x-axis at 0
plt.ylim(bottom=0) # Start y-axis at 0
plt.tight_layout()
plt.savefig('selection_method_plot.png') 
plt.show()


print("\nFinal training data size (scaled features):", train_x_scaled.shape)
print("\nFinal training data size (standardized outputs):", train_y_standardized.shape)
