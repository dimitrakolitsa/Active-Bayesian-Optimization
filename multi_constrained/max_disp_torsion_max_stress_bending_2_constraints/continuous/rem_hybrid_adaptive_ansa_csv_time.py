...
RESULTS_CSV_PATH = "moo_live_dynamic_2obj_2constr_results_V2.csv" # V2 to avoid overwriting
...
constraint_target_name_1 = "max_displacement_bending"
constraint_target_name_2 = "mass"
objective_names = [objective_target_name_1, objective_target_name_2]
constraint_names = [constraint_target_name_1, constraint_target_name_2]
target_column_names = ["mass", "max_displacement_bending", "max_displacement_torsion", "max_stress_bending", "max_stress_torsion"]

# BO Parameters
CONSTRAINT_THRESHOLD_C1 = 70.0
CONSTRAINT_THRESHOLD_C2 = 0.018
...
num_initial_samples = df_init_raw.shape[0]
...
constraint_index_c1 = target_column_names.index(constraint_target_name_1)
constraint_index_c2 = target_column_names.index(constraint_target_name_2)
obj_indices = [0, 1]
constraint_indices_in_y = [2, 3]
n_objectives = len(obj_indices)
n_constraints = len(constraint_indices_in_y)

train_y_raw = torch.cat([
    -Y_init[:, objective_index_1].unsqueeze(-1),
    -Y_init[:, objective_index_2].unsqueeze(-1),
    Y_init[:, constraint_index_c1].unsqueeze(-1),
    Y_init[:, constraint_index_c2].unsqueeze(-1)
], dim=1)

print(f"\nOptimizing objectives: Minimize {', '.join(objective_names)}")
print(f"Subject to constraints: {constraint_names[0]} < {CONSTRAINT_THRESHOLD_C1}, {constraint_names[1]} < {CONSTRAINT_THRESHOLD_C2}")
...
constraint_thresholds_raw = torch.tensor([CONSTRAINT_THRESHOLD_C1, CONSTRAINT_THRESHOLD_C2], dtype=torch.float64)
constraint_thresholds_std = torch.zeros_like(constraint_thresholds_raw)
print("\nConstraint Thresholds:")
for i in range(n_constraints):
    mean_i = y_scaler.mean_[constraint_indices_in_y[i]]
    scale_i = y_scaler.scale_[constraint_indices_in_y[i]]
    if abs(scale_i) < 1e-9:
        constraint_thresholds_std[i] = 0.0
    else:
        constraint_thresholds_std[i] = (constraint_thresholds_raw[i] - mean_i) / scale_i
    print(f"  {constraint_names[i]}: Raw={constraint_thresholds_raw[i].item():.4f}, Standardized={constraint_thresholds_std[i].item():.4f}")

# --- Adaptive BO Logic Setup ---
INITIAL_EXPLORATION_PHASE = 75 # NEW: Explicit exploration phase
alpha_variance, alpha_hv_improvement = 0.2, 0.3
smooth_avg_variance, smooth_hv_improvement = torch.tensor(0.0), torch.tensor(0.0)
stall_counter, forced_exploration_iters_left = 0, 0
N_STALL_THRESH, FORCED_EXPLORATION_DURATION = 8, 10
...
print(f"\nCalculating Initial Hypervolume...")
initial_objectives_raw = -train_y_raw[:, obj_indices]
initial_constraints_raw_c1 = train_y_raw[:, constraint_indices_in_y[0]]
initial_constraints_raw_c2 = train_y_raw[:, constraint_indices_in_y[1]]

feasible_mask_init = (initial_constraints_raw_c1 <= constraint_thresholds_raw[0]) & \
                     (initial_constraints_raw_c2 <= constraint_thresholds_raw[1])
...
print(f"Initial FEASIBLE Hypervolume: {initial_hv:.6f}, Initial Pareto Size: {initial_pareto_raw_size}")

# --- Main Optimization Block ---
script_version_name = "Live_MultiPhaseLCB_2Obj_2Constr"
...
try:
    ...
    norm = Normal(0.0, 1.0)
    forced_stall_weight_cycle = [torch.tensor([0.8, 0.2]), torch.tensor([0.2, 0.8]), torch.tensor([0.5, 0.5])]
    forced_stall_weight_idx = 0

    for i_iter_loop in range(MAX_TOTAL_ITERS):
        actual_iterations_run = i_iter_loop + 1
        iter_start_time = time.perf_counter()
        print(f"\nIteration {actual_iterations_run}/{MAX_TOTAL_ITERS}")

        bo_start_time_iter = time.perf_counter()
        model = train_independent_gps(train_x_scaled, train_y_standardized)
        model.eval()

        current_beta_lcb: float; w1: float; w2: float; rule_reason: str
        
        # --- Calculate EMA Metrics (same as before) ---
        with torch.no_grad():
            posterior_for_variance = model.posterior(train_x_scaled)
        ...
        # --- NEW REVISED: Multi-Phase Adaptive LCB Logic ---

        # Phase 1: Initial Forced Exploration
        # Force high exploration with equal weights to build a diverse understanding of the trade-off space.
        if i_iter_loop < INITIAL_EXPLORATION_PHASE:
            current_beta_lcb = 3.0
            w1, w2 = 0.5, 0.5
            rule_reason = f"Initial explore phase ({i_iter_loop+1}/{INITIAL_EXPLORATION_PHASE}, Beta={current_beta_lcb}, Equal Weights)"

        # Phase 2: Stall Recovery
        # If stalled, force high exploration but cycle weights to focus on different objectives and escape local optima.
        elif forced_exploration_iters_left > 0:
            current_beta_lcb = 3.0
            current_forced_weights = forced_stall_weight_cycle[forced_stall_weight_idx % len(forced_stall_weight_cycle)]
            w1, w2 = current_forced_weights[0].item(), current_forced_weights[1].item()
            rule_reason = f"Forced stall exploration ({forced_exploration_iters_left} left, Beta={current_beta_lcb}, Forced Weights=[{w1:.1f},{w2:.1f}])"
            forced_stall_weight_idx += 1

        # Phase 3: Main Adaptive Operation
        else:
            # 3A: If HV is not improving (stagnant), explore based on model variance.
            if smooth_hv_improvement < STALL_HV_IMPROVEMENT_TOLERANCE:
                HIGH_VARIANCE_THRESH, MODERATE_VARIANCE_THRESH = 0.5, 0.1
                ...
                rule_reason = f"Adaptive (Stagnant): Beta={current_beta_lcb} based on variance"
            
            # 3B: If HV is improving, switch to exploitation to refine the current Pareto front.
            else:
                current_beta_lcb = 1.5 # Use a lower beta to exploit promising regions
                rule_reason = f"Adaptive (Improving): Switch to exploitation with Beta={current_beta_lcb}"
            
            # In the adaptive phase, use dynamic EI-based weights to intelligently focus the search.
            with torch.no_grad():
                posterior = model.posterior(train_x_scaled) # Use train_x_scaled for more stable EI calc
            ...
            # Use the mean EI over observed points as a robust indicator
            mean_ei = torch.mean(ei_vals, dim=0) + 1e-7
            weights = mean_ei / torch.sum(mean_ei)
            
            min_weight = 0.1 # Ensure no objective is ever completely ignored
            weights = (1 - n_objectives * min_weight) * weights + min_weight
            weights = weights / torch.sum(weights)
            w1, w2 = weights[0].item(), weights[1].item()
        
        print(f"  Using LCB params: {rule_reason}, Weights=[{w1:.3f},{w2:.3f}]")

        def constrained_scalarized_lcb(X):
            posterior = model.posterior(X)
            means, stds = posterior.mean, posterior.variance.clamp_min(1e-9).sqrt()
            pof_c1 = norm.cdf((constraint_thresholds_std[0] - means[..., constraint_indices_in_y[0]]) / stds[..., constraint_indices_in_y[0]])
            pof_c2 = norm.cdf((constraint_thresholds_std[1] - means[..., constraint_indices_in_y[1]]) / stds[..., constraint_indices_in_y[1]])
            pof_total = pof_c1 * pof_c2
            lcb = (w1 * (means[..., obj_indices[0]] - current_beta_lcb * stds[..., obj_indices[0]]) +
                   w2 * (means[..., obj_indices[1]] - current_beta_lcb * stds[..., obj_indices[1]]))
            return (pof_total * lcb).squeeze(-1)

        try:
            next_x_scaled, _ = optimize_acqf(acq_function=constrained_scalarized_lcb, bounds=scaled_bounds, q=1, num_restarts=15, raw_samples=1024)
        except Exception as e:
        ...
        next_x_real_world = transform_candidate_to_real_world(next_x_unscaled_internal, bounds_unscaled, stepped_vars, categorical_vars, new_columns)
        ...
        if simulation_results is None or "error" in simulation_results:
            ...
            hypervolume_history.append(hypervolume_history[-1]); continue
        ...
        new_row_data.update({'evaluation_type': 'bo_evaluation', 'iteration_number': INITIAL_DATA_SIZE + i_iter_loop, 'bo_duration_sec': bo_duration_iter, 'iteration_duration_sec': iteration_duration_sec})
        ...
        new_obj1_raw, new_obj2_raw = simulation_results[objective_target_name_1], simulation_results[objective_target_name_2]
        new_constr1_raw, new_constr2_raw = simulation_results[constraint_target_name_1], simulation_results[constraint_target_name_2]
        print(f"  Received results: {objective_names[0]}={new_obj1_raw:.4f}, {objective_names[1]}={new_obj2_raw:.4f}")
        print(f"  Constraint values: {constraint_names[0]}={new_constr1_raw:.4f}, {constraint_names[1]}={new_constr2_raw:.6f}")

        next_y_raw = torch.tensor([[-new_obj1_raw, -new_obj2_raw, new_constr1_raw, new_constr2_raw]])
        ...
        all_objectives_min = -all_y_raw[:, obj_indices]
        all_constraints_c1, all_constraints_c2 = all_y_raw[:, constraint_indices_in_y[0]], all_y_raw[:, constraint_indices_in_y[1]]
        feasible_mask = (all_constraints_c1 <= constraint_thresholds_raw[0]) & (all_constraints_c2 <= constraint_thresholds_raw[1])
        current_objectives_feasible = all_objectives_min[feasible_mask]
        
        current_hv, current_pareto_front_size = 0.0, 0
        if current_objectives_feasible.shape[0] > 0:
            ...
            if current_pareto_front_size > 0:
                hv_calculator = Hypervolume(ref_point=negated_ref_point_for_hv_calc)
                current_hv = hv_calculator.compute(-pareto_front_raw)
    ...
    print("\n" + "="*50 + "\nAn unexpected error occurred in the main loop."); traceback.print_exc(); print("Optimization stopped prematurely.")
finally:
    if evaluator:
...
final_y_raw_all = torch.tensor(y_scaler.inverse_transform(train_y_standardized.numpy()))
final_objectives_raw_min = -final_y_raw_all[:, obj_indices]
final_constraints_raw_c1 = final_y_raw_all[:, constraint_indices_in_y[0]]
final_constraints_raw_c2 = final_y_raw_all[:, constraint_indices_in_y[1]]

final_feasible_mask = (final_constraints_raw_c1 <= constraint_thresholds_raw[0]) & \
                      (final_constraints_raw_c2 <= constraint_thresholds_raw[1])
...
final_pareto_points_raw = torch.empty((0, n_objectives), dtype=torch.float64)
...
plt.ylim(bottom=0)
plt.tight_layout()
plt.savefig('live_moo_multiphase_2obj_2constr_hypervolume.png')
print("\nSaved plot to 'live_moo_multiphase_2obj_2constr_hypervolume.png'")
plt.show()

# --- 2D Objective Space Plot ---
fig, ax = plt.subplots(figsize=(11, 9))
if final_objectives_raw_min.shape[0] > 0:
    ax.scatter(final_infeasible_objectives[:, 0].numpy(), final_infeasible_objectives[:, 1].numpy(),
               c='lightgray', alpha=0.5, s=25, label='Infeasible Points')
    ax.scatter(final_feasible_objectives[:, 0].numpy(), final_feasible_objectives[:, 1].numpy(),
               c='blue', alpha=0.6, s=35, label='Feasible Points')
    
    if final_pareto_points_raw.shape[0] > 0:
        ax.scatter(final_pareto_points_raw[:, 0].numpy(), final_pareto_points_raw[:, 1].numpy(),
    ...
    ax.set_title(f"2D Objective Space ({script_version_name})")
    ax.legend(loc='best')
    ax.grid(True)
    plt.tight_layout()
    plt.savefig('live_moo_multiphase_2obj_2constr_objective_space.png')
    print("Saved plot to 'live_moo_multiphase_2obj_2constr_objective_space.png'")
    plt.show()
else:
