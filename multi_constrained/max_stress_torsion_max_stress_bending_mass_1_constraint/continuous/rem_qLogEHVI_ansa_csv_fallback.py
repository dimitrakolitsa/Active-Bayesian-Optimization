# FILE: live_moo_bo.py
# PURPOSE: Live, Multi-Objective BO using the ANSA IAP approach with a fallback mechanism.
...
# NEW: Import Normal distribution for PoF calculation in the fallback
...
# MODIFIED: Renamed CSV to reflect new logic
RESULTS_CSV_PATH = "moo_3obj_constrained_with_fallback.csv"
...
# Add acquisition function column
...
train_c_raw = CONSTRAINT_THRESHOLD - Y_init[:, constraint_index]
...
c_scaler = StandardScaler(); c_scaler.fit(train_c_raw.numpy().reshape(-1, 1))
train_c_standardized = torch.tensor(c_scaler.transform(train_c_raw.numpy().reshape(-1, 1)), dtype=torch.float64)
...
initial_constraint_vals_raw = Y_init[:, constraint_index]
is_feasible_init = initial_constraint_vals_raw < CONSTRAINT_THRESHOLD
...
print(f"\nStarting Live 3-Objective Bayesian Optimization with 1 Constraint...")
...
norm = Normal(0.0, 1.0) # For PoF calculation
...
try:
    ...
    for i in range(MAX_TOTAL_ITERS):
        ...
        acqf_name_this_iter = ""
        constraint_index_in_model_output = n_objectives # The constraint is the last output
        ...
        if exploration_iters_left > 0:
            ...
            # The raw threshold for the constraint `C - Y` is 0. We find its standardized value.
            std_threshold_for_zero = c_scaler.transform(np.array([[0.0]]))[0, 0]

            def constrained_scalarized_lcb(X):
                posterior = model.posterior(X)
                means, stds = posterior.mean, posterior.variance.clamp_min(1e-9).sqrt()
                
                # PoF: Probability that the model output is > the standardized threshold for zero
                pof = norm.cdf((means[..., constraint_index_in_model_output] - std_threshold_for_zero) / stds[..., constraint_index_in_model_output])
                
                # LCBs for each objective (we want to maximize this)
                lcb_obj1 = means[..., 0] - BETA_EXPLORE * stds[..., 0]
                lcb_obj2 = means[..., 1] - BETA_EXPLORE * stds[..., 1]
                lcb_obj3 = means[..., 2] - BETA_EXPLORE * stds[..., 2]
                
                # Sum the LCBs to get a single exploration score
                scalarized_lcb_score = lcb_obj1 + lcb_obj2 + lcb_obj3
                
                return (pof * scalarized_lcb_score).squeeze(-1)
            ...
            constraint_callable = [lambda Z: -Z[..., constraint_index_in_model_output]] # Assumes model output > 0 is feasible
            train_y_standardized_for_partitioning = train_all_y_standardized[:, obj_indices]
            #is_feasible_for_partitioning = (train_all_y_standardized[:, -1] > c_scaler.transform(np.array([[0.0]]))[0,0])
            
            partitioning = FastNondominatedPartitioning(
                ref_point=hv_ref_point_std_neg, 
                Y=train_y_standardized_for_partitioning
            )
            acq_function_to_use = qLogExpectedHypervolumeImprovement(
                model=model, ref_point=hv_ref_point_std_neg.tolist(),
                partitioning=partitioning, objective=mc_objective,
                constraints=constraint_callable, sampler=mc_sampler,
            )
        ...
        # MODIFIED: Logic to save data now includes the acquisition function name
        if simulation_results is None or "error" in simulation_results or all(k not in simulation_results for k in objective_names):
            ...
            print(f"  Constraint value: {CONSTRAINT_TARGET_NAME}={new_constraint_val_raw:.4f} (Feasible: {new_constraint_val_raw < CONSTRAINT_THRESHOLD})")
            ...
            next_c_raw = torch.tensor([[CONSTRAINT_THRESHOLD - new_constraint_val_raw]])
            next_c_standardized = torch.tensor(c_scaler.transform(next_c_raw.numpy()), dtype=torch.float64)
            next_all_y_standardized = torch.cat([next_y_standardized, next_c_standardized], dim=1)
            
            # Use the REAL WORLD point for retraining
            ...
            constraint_residuals_raw = torch.tensor(c_scaler.inverse_transform(train_all_y_standardized[:, -1].unsqueeze(-1).numpy())).squeeze()
            
            is_feasible_mask = constraint_residuals_raw > 0
...
final_constraint_residuals_raw = torch.tensor(c_scaler.inverse_transform(train_all_y_standardized[:, -1].unsqueeze(-1).numpy()), dtype=torch.float64).squeeze()
final_is_feasible_mask = final_constraint_residuals_raw > 0
...
ax1.set_title("Feasible Hypervolume Convergence Plot (3 Objectives, 1 Constraint)")
ax1.grid(True, which='both', linestyle='--')
ax1.set_xlim(left=0)
ax1.set_ylim(bottom=0)
plt.tight_layout()
plt.savefig('live_moo_3obj_constrained_hypervolume_with_fallback.png')
print("\nSaved plot to 'live_moo_3obj_constrained_hypervolume_with_fallback.png'")
plt.show()
...
if final_objectives_raw.shape[0] > 0:
    ax2.scatter(final_infeasible_objectives_raw[:, 0].numpy(),
                final_infeasible_objectives_raw[:, 1].numpy(),
                final_infeasible_objectives_raw[:, 2].numpy(),
                c='lightgray', alpha=0.5, s=15, label='Infeasible Points')
    ax2.scatter(final_feasible_objectives_raw[:, 0].numpy(),
                final_feasible_objectives_raw[:, 1].numpy(),
                final_feasible_objectives_raw[:, 2].numpy(),
                c='blue', alpha=0.6, s=25, label='Feasible Points')
    if final_pareto_points_raw.shape[0] > 0:
        ax2.scatter(final_pareto_points_raw[:, 0].numpy(),
                    final_pareto_points_raw[:, 1].numpy(),
                    final_pareto_points_raw[:, 2].numpy(),
                    c='lime', s=150, edgecolor='black', marker='*', label='Final Feasible Pareto Front', depthshade=False, zorder=3)
    ...
    ax2.set_title("3D Objective Space - Live Optimization Results (with Constraint)")
    ax2.legend(loc='best')
    plt.tight_layout()
    plt.savefig('live_moo_3obj_constrained_objective_space_with_fallback.png')
    print("Saved plot to 'live_moo_3obj_constrained_objective_space_with_fallback.png'")
    plt.show()
else:
