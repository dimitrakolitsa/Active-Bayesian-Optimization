...
# BoTorch
from botorch.models.model import Model # Base class for custom models
...
# GPyTorch
import gpytorch
from gpytorch.distributions import MultivariateNormal # For custom model posterior
from gpytorch.mlls import ExactMarginalLogLikelihood # For GPs
from gpytorch.kernels import MaternKernel, ScaleKernel # For GPs
from gpytorch.priors import GammaPrior, SmoothedBoxPrior # For GPs
...
# --- Custom XGBoost Model Definition ---
class CustomXGBoostModel(Model):
    """
    A custom XGBoost ensemble model for BoTorch.
    Fits an ensemble of XGBoost regressors for each output dimension.
    Variance is estimated from the ensemble predictions.
    """
    def __init__(self,
                 train_X: torch.Tensor,
                 train_Y: torch.Tensor,
                 num_ensemble_models: int = 20, # Number of XGBoost models in the ensemble per output
        ...
        self.ensembles = []  # List of ensembles (one ensemble per output)
                             # Each ensemble is a list of fitted XGBoost models

        base_xgb_params = xgb_params or {}
        # Ensure a default objective if not specified
        if 'objective' not in base_xgb_params:
            base_xgb_params['objective'] = 'reg:squarederror'


        for i_output in range(self._num_outputs):
            output_ensemble = []
            for i_member in range(num_ensemble_models):
                # Create bootstrap sample
                # Ensure n_samples matches original, can set replace=True
                X_boot, Y_boot_output = resample(
                    train_X_np,
                    train_Y_np[:, i_output],
                    replace=True, #bootstrapping => initial data has 50 points and the replace=true creates a new 50-point dataset by randomly picking points from the original set with replacement
                    n_samples=train_X_np.shape[0],
                    random_state=np.random.RandomState(i_output * num_ensemble_models + i_member) # Vary random state for bootstrap
                )

                # Vary random_state for XGBoost model itself for additional diversity
                current_xgb_params = base_xgb_params.copy()
                current_xgb_params['random_state'] = i_output * num_ensemble_models + i_member

                model = xgboost.XGBRegressor(**current_xgb_params)
                try:
                    model.fit(X_boot, Y_boot_output)
                except Exception as e:
                    print(f"Warning: XGBoost fitting failed for output {i_output}, member {i_member}: {e}")
                    continue
                output_ensemble.append(model)
            
            if not output_ensemble: # If all members failed for an output
                raise RuntimeError(f"All XGBoost ensemble members failed to train for output {i_output}.")
            self.ensembles.append(output_ensemble)
    ...
    def train(self, mode: bool = True):
        ...
        if mode: # Entering training mode
            self._is_trained = False # Indicate it might need retraining if data changes
    ...
    def posterior(self, X: torch.Tensor) -> GPyTorchPosterior:
        if not self._is_trained:
            # This basic check might not be enough if data truly changes,
            # as the model is not retrained by calling train(True) then train(False)
             warnings.warn("CustomXGBoostModel is in training mode or not marked as trained. Ensure it's fitted before calling posterior.", UserWarning)
        ...
        X_np = X_reshaped.cpu().detach().numpy()
        n_total_samples_reshaped = X_reshaped.shape[0]

        means_list = []
        variances_list = []

        for i_output in range(self._num_outputs):
            output_ensemble = self.ensembles[i_output]
            if not output_ensemble: # Should have been caught in init, but good check
                 raise RuntimeError(f"No valid XGBoost models in ensemble for output {i_output}")


            member_predictions_list = []
            for model in output_ensemble:
                try:
                    preds = model.predict(X_np)
                    member_predictions_list.append(preds)
                except Exception as e:
                    print(f"Warning: Prediction failed for an XGBoost member (output {i_output}): {e}. Skipping member.")
                    # Handle by predicting NaN or a safe default if necessary, then filter out
                    member_predictions_list.append(np.full(n_total_samples_reshaped, np.nan))


            # Filter out NaNs before converting to array, if any member failed prediction
            valid_predictions = [p for p in member_predictions_list if not np.isnan(p).any()]
            if not valid_predictions:
                # All members failed prediction for this X
                warnings.warn(f"All XGBoost members failed to predict for output {i_output}. Returning zero mean and high variance.", UserWarning)
                current_means_np = np.zeros(n_total_samples_reshaped)
                current_variances_np = np.ones(n_total_samples_reshaped) * 1e6 # High variance
            else:
                member_predictions_np = np.array(valid_predictions) # Shape: (num_valid_ensemble_models, n_total_samples_reshaped)
                current_means_np = np.mean(member_predictions_np, axis=0)
                current_variances_np = np.var(member_predictions_np, axis=0)

            means_list.append(torch.from_numpy(current_means_np).to(X))
            variances_list.append(torch.from_numpy(current_variances_np).to(X))

        means_stacked = torch.stack(means_list, dim=-1)
        variances_stacked = torch.stack(variances_list, dim=-1)

        if len(original_batch_shape) > 0:
            means = means_stacked.reshape(*original_batch_shape, self._num_outputs)
            variances = variances_stacked.reshape(*original_batch_shape, self._num_outputs)
        else:
            means = means_stacked
            variances = variances_stacked
...
# --- BO Loop Setup ---
n_iterations = 300 
CURRENT_MODEL_TYPE = "XGBoost" # "GP", "RF", or "XGBoost"

# Parameters for CustomXGBoostModel
XGB_ENSEMBLE_SIZE = 25 #10 Number of XGBoost models in the ensemble per output
XGB_PARAMS = {         # Parameters for each XGBoost member model
    'objective': 'reg:squarederror',
    'n_estimators': 150,  #each XGB model is a forest of 150 trees
    'learning_rate': 0.05, #How slowly each new tree corrects the previous ones
    'max_depth': 6, #max depth of any single DT
    'subsample': 0.8, # Each tree is trained on a random 80% of the data rows
    'colsample_bytree': 0.8, # Each tree uses a random 80% of the features
    'gamma': 0.1, #for regularization to prevent overfitting
    # 'random_state' will be set internally per member for diversity
...
script_version_name = f"LCB_AdaptiveBeta_DynamicEarlyExplore_{CURRENT_MODEL_TYPE}"
print(f"\nStarting Bayesian Optimization for {n_iterations} iterations ({script_version_name})...")
print(f"  Using Surrogate Model: {CURRENT_MODEL_TYPE} (Ensemble size per output: {XGB_ENSEMBLE_SIZE})")
print(f"  XGBoost member parameters: {XGB_PARAMS}")
# ... (rest of the print statements for BO setup) ...
...
for i_iter_loop in range(n_iterations):
    ...
    try:
        if CURRENT_MODEL_TYPE == "XGBoost":
            model = CustomXGBoostModel(
                train_X=train_x_scaled,
                train_Y=train_y_standardized,
                num_ensemble_models=XGB_ENSEMBLE_SIZE,
                xgb_params=XGB_PARAMS
            )
        # Add elif for "GP" or "RF" if you want to switch easily later
        # elif CURRENT_MODEL_TYPE == "GP":
        #    # model = train_gp_model(...) # You'd need to define this
        #    pass
        else:
    ...
    # ... (rest of the BO loop starting from "Using Constrained Scalarized LCB...")
    # The following parts of your BO loop should work as long as `model.posterior(X)`
    # returns an object with .mean and .variance attributes.
    ...
    if i_iter_loop == 0:
        ...
        if len(hypervolume_history) > 1 and (hypervolume_history[-1] - hypervolume_history[-2]) != 0 :
             smooth_hv_improvement = (hypervolume_history[-1] - hypervolume_history[-2]) # Already a tensor
        else:
             smooth_hv_improvement = torch.tensor(0.0, dtype=torch.float64, device=X_init.device)
    ...
    if forced_exploration_iters_left > 0:
        if is_late_stage_iteration:
            ...
            current_beta_lcb = beta_states[5].item() # Max exploration beta
            ...
            rule_reason = f"Forced stall exploration ({forced_exploration_iters_left} iters left, Beta {current_beta_lcb:.1f}, Forced Stall W=[{w1:.1f},{w2:.1f},{w3:.1f}])"
            forced_stall_weight_idx += 1
            use_dynamic_ei_weights_for_this_iteration = False
    else:
        if DYNAMIC_EARLY_EXPLORE_ITERATIONS is None: # Fallback, should be set
             DYNAMIC_EARLY_EXPLORE_ITERATIONS = int(n_iterations * 0.30)
             print("    Warning: DYNAMIC_EARLY_EXPLORE_ITERATIONS was not set (should be set in iter 0), using default fallback.")

        if i_iter_loop < DYNAMIC_EARLY_EXPLORE_ITERATIONS:
            current_beta_lcb = beta_states[5].item() # 3.0 Max exploration beta
            ...
            if smooth_avg_variance > HIGH_VARIANCE_THRESH:
                current_beta_lcb = beta_states[5].item() # 3.0
                _rule_reason_beta_part_for_print = "EMA Stagnant, high variance (Beta 3.0)"
            elif smooth_avg_variance > MODERATE_VARIANCE_THRESH:
                current_beta_lcb = beta_states[4].item() # 2.5
                _rule_reason_beta_part_for_print = "EMA Stagnant, moderate variance (Beta 2.5)"
            else: # low variance
                current_beta_lcb = beta_states[1].item() # 1.0
                _rule_reason_beta_part_for_print = "EMA Stagnant, low var -> precise exploitation (Beta 1.0)"
        elif smooth_avg_variance > HIGH_VARIANCE_THRESH: # Decent HV improvement implied
            current_beta_lcb = beta_states[3].item() # 2.0
            _rule_reason_beta_part_for_print = "Decent improv (implied), high variance (Beta 2.0)"
        elif i_iter_loop > MID_ITER_MAX_EMA: # Late stage, decent HV improvement, moderate/low variance
            if smooth_avg_variance < LOW_VARIANCE_THRESH:
                current_beta_lcb = beta_states[0].item() # 0.5
                _rule_reason_beta_part_for_print = "Late stage, low variance -> exploit (Beta 0.5)"
            elif smooth_avg_variance < MODERATE_VARIANCE_THRESH:
                current_beta_lcb = beta_states[1].item() # 1.0
                _rule_reason_beta_part_for_print = "Late stage, mod-low variance -> mild exploit (Beta 1.0)"
            else: # MODERATE_VARIANCE_THRESH <= smooth_avg_variance <= HIGH_VARIANCE_THRESH
                current_beta_lcb = beta_states[2].item() # 1.5
                _rule_reason_beta_part_for_print = "Late stage, default balanced (Beta 1.5)"
            ...
            if block_phase_alternating == 0: # Explore a bit more
                current_beta_lcb = beta_states[3].item() # 2.0
            else: # Exploit a bit more
                current_beta_lcb = beta_states[2].item() # 1.5
    ...
    if use_dynamic_ei_weights_for_this_iteration:
        ...
        if sum_S_eff <= (n_objectives * epsilon_score):
        ...
        if forced_exploration_iters_left > 0 and is_late_stage_iteration:
    ...
    _beta_constraint_eff: float # Temporary variable for clarity in logic

    if forced_exploration_iters_left > 0:
        if is_late_stage_iteration: # Late stage forced exploration
            _beta_constraint_eff = 2.0 # Still quite conservative
            print(f"    Using CONSERVATIVE constraint beta: {_beta_constraint_eff:.1f} (Forced Exploration - Late Stage)")
        else: # Early/Mid stage forced exploration
            _beta_constraint_eff = 2.5 # VERY conservative
            print(f"    Using VERY CONSERVATIVE constraint beta: {_beta_constraint_eff:.1f} (Forced Exploration - Early/Mid Stage)")
    else: # Normal operation (not forced exploration)
        _beta_constraint_eff = 1.5 # Significantly more conservative than 1.0 or 1.5
        print(f"    Using Conservative constraint beta: {_beta_constraint_eff:.1f} (Normal Operation)")

    # Ensure beta_constraint_eff is a tensor for multiplication if std_constr_std is a tensor
    beta_constraint_eff_tensor = torch.tensor(_beta_constraint_eff, device=mean_constr_std.device, dtype=mean_constr_std.dtype)
    constraint_lcb_values = mean_constr_std - beta_constraint_eff_tensor * std_constr_std
    ...
    next_y_standardized = torch.tensor(y_scaler.transform(next_y_combined_raw_for_model.numpy()), dtype=torch.float64) # Use .numpy() for scaler
    ...
    all_y_raw_from_standardized = torch.tensor(y_scaler.inverse_transform(train_y_standardized.cpu().numpy()), dtype=torch.float64) # ensure cpu for scaler
    ...
    if forced_exploration_iters_left > 0 and rule_reason.startswith("Forced stall exploration"): # Ensure we only decrement if forced exploration actually happened
...
# --- End of BO Loop ---
# ... (The rest of your script for results, plotting, etc. should largely remain the same) ...
# Make sure to update plot titles/suffixes if `plot_suffix` uses `script_version_name` which includes CURRENT_MODEL_TYPE.
...
final_y_all_raw_model_form = torch.tensor(y_scaler.inverse_transform(final_y_all_standardized.cpu().numpy()), dtype=torch.float64) # ensure cpu
...
try:
    precision_dup = 1e-5
    rounded_objectives = np.round(true_space_objectives_feasible_min.cpu().numpy() / precision_dup) * precision_dup # ensure cpu
...
if unique_true_space_objectives_min is not None and unique_true_space_objectives_min.shape[0] > 0:
    ...
    if true_pareto_front_raw_minimization.shape[0] > 0:
        ...
        if true_hv > 1e-12: print(f"HV Ratio (BO/True): {final_bo_hv / true_hv:.4f}")
else:
    print("Could not determine true Pareto front (no unique feasible points or error).")

print("\n--- Checking Overlap Between BO Front and True Front (Exact Float Comparison, Minimization Objectives) ---")
if final_bo_pareto_points_raw_minimization.shape[0] > 0 and true_pareto_front_raw_minimization.shape[0] > 0:
    ...
    print(f"  - BO found {final_bo_pareto_points_raw_minimization.shape[0]} Pareto points (minimization).")
    print(f"  - True Pareto Front contains {true_pareto_front_raw_minimization.shape[0]} points (minimization).")
    print(f"  - {match_count} BO Pareto points (rounded to 6dp for matching) match points on the True Pareto Front.")

    if match_count < true_pareto_front_raw_minimization.shape[0] and true_pareto_front_raw_minimization.shape[0] > 0:
        print("\n--- Missed True Pareto Points (objectives rounded to 6dp for identification) ---")
        ...
        for true_pt in true_pareto_front_raw_minimization:
            ...
            if true_pt_tuple_rounded not in bo_points_tuples_set:
                missed_true_points.append(true_pt.cpu().numpy())

        if missed_true_points:
            df_missed = pd.DataFrame(np.array(missed_true_points), columns=objective_names)
            print(df_missed.to_string())
        else:
            print("  Could not identify missed points due to potential float precision complexities in set difference.")
...
if len(selection_methods) > 0:
    ...
    bo_hypervolumes_plot = hypervolume_history[1:len(selection_methods)+1] if len(hypervolume_history) > len(selection_methods) else hypervolume_history[1:]
...
if true_pareto_front_raw_minimization.shape[0] > 0 and true_hv > 1e-9: plt.axhline(y=true_hv, color='green', linestyle='--', label=f'True Max HV ({true_hv:.4f})')
...
plt.grid(True)
plt.xlim(left=0)
if len(bo_hypervolumes_plot) > 0 : plt.ylim(bottom=min(0, min(bo_hypervolumes_plot) * 1.1 if min(bo_hypervolumes_plot) < 0 else min(bo_hypervolumes_plot) * 0.9 ))
plt.tight_layout()
...
if final_objectives_raw_minimization.shape[0] > 0:
    ...
    if final_bo_pareto_points_raw_minimization.shape[0] > 0:
        ax.scatter(final_bo_pareto_points_raw_minimization[:,0].cpu().numpy(), final_bo_pareto_points_raw_minimization[:,1].cpu().numpy(), final_bo_pareto_points_raw_minimization[:,2].cpu().numpy(), c='lime', s=150, edgecolor='black', marker='*', label=f'BO Pareto ({plot_suffix})', zorder=3)
    if true_pareto_front_raw_minimization.shape[0] > 0:
