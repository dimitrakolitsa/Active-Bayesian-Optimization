...
print(f"Will stop if relative improvement is less than {REL_IMPROVEMENT_TOL:.1e} over a {PATIENCE_WINDOW}-iteration window.")
...
for i in range(MAX_TOTAL_ITERS):
    ...
    if i >= BASE_ITERS - 1:
        # Check if we have enough history and have found a feasible solution in the past window
        if len(best_feasible_values) > PATIENCE_WINDOW and best_feasible_values[-(PATIENCE_WINDOW + 1)] != float('inf'):
            ...
            # Check for division by zero if past_best is 0
            if abs(past_best) > 1e-9:
                ...
                if relative_improvement < REL_IMPROVEMENT_TOL:
                    ...
                    break
            # If past value is near zero, use a small absolute check as a fallback
...
# Add an else block to the for loop to catch the case where it completes without breaking
else:
...
final_objective_values_raw = final_y_raw[:, 0]  # Raw objective (positive)
final_mass_values_raw = final_y_raw[:, 1]  # Raw mass
feasible_final_mask = final_mass_values_raw < constraint_threshold

if feasible_final_mask.any():
    final_objective_values_negated = -final_objective_values_raw  # Restore original scale
    ...
    # Convert feasible_final_mask to a tensor for torch.where
...
iterations = list(range(len(best_feasible_values))) # 0 to n_iter
num_bo_iters = len(selection_methods) # number of successful BO iterations run
...
# plot initial point (Iteration 0)
if len(iterations) > 0:
    plt.scatter(iterations[0], best_feasible_values[0], c='black', marker='s', s=60, label='Initial Best', zorder=3, edgecolors='grey') # Add outline

# plot points from BO iterations (Iterations 1 onwards)
if num_bo_iters > 0:
    plot_bo_iterations = iterations[1 : num_bo_iters + 1]
    plot_bo_values = best_feasible_values[1 : num_bo_iters + 1]
    # red for logcei and blue for lcb fallback
...
hline_labels = {}
if actual_min_feasible_objective_raw != float('inf'):
    plt.axhline(y=actual_min_feasible_objective_raw, color='#FF7F00', linestyle=':', linewidth=2, label=f'Actual Min Feasible ({actual_min_feasible_objective_raw:.6f})', alpha=0.9) # Orange dashed
    hline_labels['Actual Min Feasible'] = f'{actual_min_feasible_objective_raw:.6f}'
if best_bo_objective_raw != float('inf'):
    plt.axhline(y=best_bo_objective_raw, color='#4DAF4A', linestyle=':', linewidth=2, label=f'BO Min Feasible ({best_bo_objective_raw:.6f})', alpha=0.9) # Green dashed
...
legend_elements = [
    ...
    Line2D([0], [0], marker='o', color='w', label='LogCEI Selection', markerfacecolor='#E41A1C', markeredgecolor='grey', markersize=8), # Red
    Line2D([0], [0], marker='o', color='w', label='LCB Fallback Selection', markerfacecolor='#377EB8', markeredgecolor='grey', markersize=8), # Blue
]
