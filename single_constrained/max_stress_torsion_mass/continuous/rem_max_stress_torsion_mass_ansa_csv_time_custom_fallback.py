...
RESULTS_CSV_PATH = "lcb_fallback_constrained_bo_results.csv"
...
# Parameters for stagnation detection and exploration
...
CSV_HEADER = new_columns + target_column_names + ['acquisition_function', 'evaluation_type', 'iteration_number', 'duration_seconds', 'bo_selection_time']
...
train_y_standardized = torch.tensor(y_scaler.fit_transform(train_y_raw.numpy()), dtype=torch.float64)
...
evaluator = None
try:
    evaluator = AnsaRemoteEvaluator(
        project_dir=PROJECT_DIR,
        ansa_worker_script=ANSA_WORKER_SCRIPT
    )
    ...
    for i in range(MAX_TOTAL_ITERS):
        ...
        #Variable to store the name of the acquisition function for logging
        acqf_name_this_iter = ""
        ...
        if exploration_iters_left > 0:
            ...
            if feasible_mask.any():
                ...
                best_f_standardized = train_y_standardized[:, 0].min().item() - 3.0
            
            constraints = {1: (None, constraint_threshold_standardized)}
            acq_function_to_use = LogConstrainedExpectedImprovement(
                model, best_f=best_f_standardized, objective_index=0, constraints=constraints
            )
        ...
        if simulation_results is None or "error" in simulation_results:
            ...
            #Add acquisition function name to the failed row data
        ...
        #Add acquisition function name to the successful row data
        new_row_data.update({
            'acquisition_function': acqf_name_this_iter,
...
if np.any(feasible_mask):
    ...
    best_x_internal = torch.tensor(x_scaler.inverse_transform(train_x_scaled[best_idx_in_train].reshape(1, -1)))
...
plt.tight_layout()
plt.savefig(f'live_constrained_bo_progress_with_lcb_fallback.png')
print("\nSaved progress plot to 'live_constrained_bo_progress_with_lcb_fallback.png'")
plt.show()